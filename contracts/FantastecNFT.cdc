...
pub contract FantastecNFT: NonFungibleToken {
  ...
  pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver {
    ...
    access(self) fun getCardCollectionRedeemInfo(): FantastecSwapDataProperties.RedeemInfo? {
      let card = self.getCard()
      let cardMetadata = card?.metadata ?? {}
      let redeemInfoMetadata = cardMetadata["redeemInfo"]
      if (redeemInfoMetadata != nil && redeemInfoMetadata!.length > 0) {
        let redeemInfo = redeemInfoMetadata![0] as? FantastecSwapDataProperties.RedeemInfo
        return redeemInfo
      }
      return nil
    }
    ...
    pub fun resolveView(_ view: Type): AnyStruct? {
      switch (view) {
        case Type<MetadataViews.Display>():
          ...
          let name = card!.name
          let cardCollection = self.getCardCollection()
          var description = name.concat(", ").concat(cardCollection!.title)
          let level = self.getNFTLevel()
          if (level != nil) {
            description = description.concat(", ").concat(level!.name)
          }
          var thumbnail = self.getCardCollectionMediaFile("CARD_THUMBNAIL")
        ...
        case Type<MetadataViews.Editions>():
          ...
          let cardCollection = self.getCardCollection()
          let max = self.getNFTMintVolume() ?? 0
